...
import com.lx.talcar.databinding.FragmentPhoto1Binding
...
class FragmentPhoto1 : Fragment() {
    ...
    private  var _binding: FragmentPhoto1Binding? =null
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        _binding = FragmentPhoto1Binding.inflate(inflater, container, false)
        ...
        _binding?.frontPhotoTab?.setOnClickListener {
            takePicture()
        }
    ...
    fun takePicture() {
        Intent(MediaStore.ACTION_IMAGE_CAPTURE).also { captureIntent ->
            ...
            println("takePicture :$outputFile")
            ...
            println("providerFile :$providerFile")
            captureIntent.resolveActivity(mainActivity.packageManager)?.also {
                println("captureIntent : $captureIntent")
    ...
    fun handleCaptureResult() {
        ...
        println("outputFile.absolutePath : ${outputFile.absolutePath}")
    ...
    private fun loadImageFromFile(imagePath: String) {
        Log.d("loadImage", "path : $imagePath")
        val targetWidth: Int = _binding?.frontPhotoTab?.width!!
        val targetHeight: Int = _binding?.frontPhotoTab?.height!!
        ...
        BitmapFactory.decodeFile(imagePath, bmOptions)?.also { bitmap ->
            _binding?.frontPhotoTab?.setImageBitmap(bitmap)
        }
