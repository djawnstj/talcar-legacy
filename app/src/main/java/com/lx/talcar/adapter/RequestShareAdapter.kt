...
class RequestShareAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    private var listener : RequestShareAdapter.OnItemClickListener? = null
    fun setOnItemClickListener(listener : RequestShareAdapter.OnItemClickListener) {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        if (holder is com.lx.talcar.adapter.RequestShareAdapter.Holder) {
            val item = items[position]
            holder.setItem(item)
        }
        //onCreateViewHolder에서 리턴받은 뷰홀더가 Holder2라면 상대의 채팅, item_your_chat의 뷰들을 초기화 해줌
        else if(holder is com.lx.talcar.adapter.RequestShareAdapter.Holder2) {
            val item = items[position]
            holder.setItem(item)
        }
        else if(holder is com.lx.talcar.adapter.RequestShareAdapter.Holder3) {
            val item = items[position]
            holder.setItem(item)
        }
        else if(holder is com.lx.talcar.adapter.RequestShareAdapter.Holder4) {
            val item = items[position]
            holder.setItem(item)
        }
        else if(holder is com.lx.talcar.adapter.RequestShareAdapter.Holder5) {
            val item = items[position]
            holder.setItem(item)
        }
    }
